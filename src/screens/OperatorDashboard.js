...
export default function OperatorDashboard({ navigation }) {
  ...
  // Estado focado no ID para evitar loops de re-renderização na base de dados
  const [selectedIncidentId, setSelectedIncidentId] = useState(null);

  // Encontra dinamicamente os metadados do incidente selecionado
  const selectedIncident = todasOcorrencias.find(r => r.id === selectedIncidentId);

  // 1. Escuta a lista global de ocorrências SOS
  useEffect(() => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* BOTÃO DE PERFIL FLUTUANTE (Visível apenas fora da sala de chat) */}
      {!selectedIncidentId && (
        <TouchableOpacity 
          style={{
            position: 'absolute',
      ...
      {!selectedIncidentId ? (
        ...
        selectedIncident && (
          ...
          >
            ...
            {/* Renderização condicional do componente Chat ou do Histórico Arquivado */}
            <View style={{ flex: 1 }}>
